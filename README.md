...
```
ID_КАНАЛА - это ID вашего Telegram-канала. Откройте веб версию Telegram, в разделе `Все чаты` найлите ваш канал. Скопруйте часть ссылке после символа `#`. Полученный текст и есть ID-канала. Выглядит примерно так: `-4730435778`
...
```
ТОКЕН_БОТА - это токен вашего Telegram-бота. Чтобы его получить, выполните команду `/token` в чате с [BotFather](https://telegram.me/BotFather)
