...
В директории проекта присутствут папки и файлы, которые пользователю не нужно открывать, они носят технический характер и не понадобятся для работы. В этот список входят:
1. `.git`
2. `__pycache__`
3. `.gitignore`
4. `http_utils.py`
