...
Проект предназначен для скачивания фотографий космической тематики сделаных в таких компаниях как SpaceX и NASA, а также для публикации полученных фотографий в Telegram-канале c помощью бота.
...
```
python /путь/к/вашему/файлу/publish_image.py
```
### Пример
Если у вас есть файл `publish_image.py`, находящийся в папке `Documents/project`, вы бы сделали следующее:
```
...
```
`API_ТОКЕН_NASA` - это ваш токен для работы с API NASA. Получить этот токен можно на [сайте NASA](https://api.nasa.gov/) после заполнения небольшой анкеты. Токен выглядит примерно так: `FS24Pz7wJw3fMcXa1GKoKh6CJacAGPZJLJ4AJj8w`
```
...
```
IMAGES_PUBLICATION_FREQUENCY=ПЕРИОДИЧНОСТЬ_ПУБЛИКАЦИЙ_КАРТИНОК
```
`ПЕРИОДИЧНОСТЬ_ПУБЛИКАЦИЙ_КАРТИНОК` - это численное значение в секундах, указывающее на периодичность публикации фотографий. Если написать `5`, то бот будет публиковать фотографии раз в 5 секунд. Если написать `14400`, то публикация будет раз в 4 часа.

Таким образом содержание файла `.env` должно выглядеть примерно так:
```
NASA_API_KEY=FS24Pz7wJw3fMcXa1GKoKh6CJacAGPZJLJ4AJj8w
TELEGRAM_CHANNEL_ID=-4730435778
TELEGRAM_BOT_TOKEN=7747985446:AAFuTe2Sm8vMEbNToAj-G60RyGtIF9amMSD
IMAGES_PUBLICATION_FREQUENCY=14400
```
...
Изначально в директории проекта присутствут папки и файлы, которые пользователю не нужно открывать, они носят технический характер и не понадобятся для работы, при этом их лучше не трогать. В этот список входят:
1. `__pycache__`
2. `.gitignore`
3. `publish_image.py`
...
### publish_random_or_selected_image.py
Если выполнить команду:
```
python publish_random_or_selected_image.py nasa_apod_images
```
...
### cycle_publish_images.py
Если выполнить команду:
```
python cycle_publish_images.py nasa_apod_images
```
то Telegram-бот опубликует на канале поочередно все фотографии из папки `nasa_apod_images`. Как и в предыдущем скрипте, можно указать другую папку. Периодичность публикаций можно указать в файле `.env`, как уже было сказано ранее. После того, как все фотографии из папки будут опубликованы, начнутся бесконечные циклы публикаций. Один цикл будет включать в себя публикацию всех фотографий папки в случайном порядке. 
